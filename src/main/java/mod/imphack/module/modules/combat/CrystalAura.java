...
@EventHandler
private final Listener<ImpHackEventPacket.ReceivePacket> packetReceiveListener = new Listener<>(event -> {
...
@EventHandler
private final Listener<ImpHackEventPacket.ReceivePacket> packetReceiveListener2 = new Listener<>(event -> {
    if (event.get_packet() instanceof SPacketSoundEffect) {
        final SPacketSoundEffect packet = (SPacketSoundEffect) event.get_packet();
        if (packet.getCategory() == SoundCategory.BLOCKS && packet.getSound() == SoundEvents.ENTITY_GENERIC_EXPLODE) {
...
private List<BlockPos> findCrystalBlocks() {
    ...
    positions.addAll(getSphere(getPlayerPos(), placeRange.getValue(), (int)placeRange.getValue(), false, true, 0).stream().filter(this::canPlaceCrystal).collect(Collectors.toList()));
    return positions;
}
...
private boolean validTarget(Entity entity) {
    if ((entity == null) || !(entity instanceof EntityLivingBase) || entity.isDead || ((EntityLivingBase) entity).getHealth() <= 0.0F)
        return false;
