...
public class Criticals extends Module {
	...
	private final Listener<ImpHackEventPacket.SendPacket> listener = new Listener<>(event -> {
		if (event.get_packet() instanceof CPacketUseEntity) {
			if (mc.player != null && ((CPacketUseEntity) event.get_packet()).getAction() == CPacketUseEntity.Action.ATTACK && packet.getEntityFromWorld(this.mc.world) instanceof EntityLivingBase && mc.player.onGround && !mc.player.isInLava() && !mc.player.isInWater()) {
